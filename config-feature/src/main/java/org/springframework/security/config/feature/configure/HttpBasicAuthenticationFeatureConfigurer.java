 ...
 * @author Joe Grandja
 */
public final class HttpBasicAuthenticationFeatureConfigurer implements FeatureConfigurer {

	@Override
	public void configure(FeatureConfigurationContext configurationContext) {
		if (!configurationContext.getFeature().getId().equals(HttpBasicAuthenticationFeature.FEATURE_ID)) {
			return;
		}

		HttpBasicAuthenticationFeature httpBasicAuthenticationFeature = configurationContext.getFeature();
